...
| FR-3 UDP port match | Spec text (§6.1) reads as implemented | **Unimplemented until D49.** `cfg_udp_port`/register `0x40` was not consumed anywhere in the datapath — no destination-UDP-port comparison existed in `rtl/` or the vendored MAC (D19). Any UDP datagram reaching the board's IP, on any port, whose length is a multiple of 16, was parsed as market data. `err_udp_port` and `err_ethertype` (§10) were structurally unable to increment as a direct consequence. **Implemented (D49)**: `udp_rx.v` exposes the destination port (`udp_rec_dest_port`), `frame_classifier.v` discards any frame whose port != `cfg_udp_port` (wired from csr_block 0x40) and pulses `err_udp_port`; `mac_rx.v` pulses `err_ethertype` for a non-`0x0800`/`0x0806` EtherType. See `docs/contracts/fr3_udp_port_match.md` |
| §11.4 `T26_soak` | Read as a 1,000,000-message full-datapath comparison | **Measured infeasible at 1,000,000 messages for the full datapath, by design going forward (D51).** The 1,000,000-message soak that actually runs (`tb_parser_soak`, S2 gate) covers only `frame_classifier`+`md_parser` (~3 min) — no book, features, ML, risk, or egress; a literal 1,000,000-message *full-datapath* run was timed directly at 200,000 messages (16m47s) and extrapolates to ~80-90 minutes, untenable for routine `make sim`/CI. The full-datapath comparison (`tb/tb_top.v`) is raised from 209 to **5,000 messages** (D51, ~15-30s) — a real ~24x increase in state-space coverage, not the literal spec figure. Five latent golden-model timing bugs were found and fixed extending it this far (D51); one dense-burst `cnt_order_overflow` discrepancy (11 real vs. 15 predicted at seed=7) remains open and is reported as a warning, not a hard failure, in `tb_top.v` |
| §10 counter invariants | Read as "asserted in the testbench" | Not mechanically asserted anywhere as of D44. Invariant 1 (`cnt_msgs_rx = filtered + accepted + Σerr_*`) does not currently hold on its own terms either: `csr_block.v` taps the same byte stream as market data (D19), so every `0x20`/`0x21` CSR frame also increments `cnt_frames_rx`/`cnt_msgs_rx`/`err_msg_type` and (via `seq_monitor.v`) `cnt_seq_dup` — `tb_top.v`'s own expected-counter file only balances because `sim/gen_top_soak_vectors.py` models this exactly, not because the invariant is architecturally true. Separating the CSR read path from ingress counting and adding the five invariants as real testbench assertions are bundled as one pre-S11 contract (they're causally linked: the invariants can't hold, and therefore can't be asserted, until the CSR path stops polluting ingress counters). **Implemented**: CSR frames (0x20/0x21) are excluded from `err_msg_type` in `md_parser.v`, so neither `csr_block.v`'s message-level counters nor `seq_monitor.v` ever see them (D48). Invariants 1/3/4/5 asserted as written; **invariant 2 is asserted in its corrected reject-frame-conservation form** — §10's literal text (signals = orders + Σ(cnt_rej_*)) double-counts multi-gate rejects because FR-43 mandates every fired gate's counter increments; see the §10 note / D48 |
...
cnt_signal_buy + cnt_signal_sell = cnt_orders_tx + cnt_order_overflow + cnt_reject_decisions
...
**Invariant 2 correction (found implementing `docs/contracts/csr_ingress_separation.md`'s S4, 2026-09-09): `Σ(cnt_rej_*)` is not the right term.**
`cnt_rej_*` are genuinely "one per gate" counters (FR-43: "every triggered gate's counter increments" — verified, tested behavior, `tb_risk_engine.v` block G/T21) — so on any intent where multiple gates fire simultaneously, `Σ(cnt_rej_*)` counts that ONE rejected intent multiple times (once per gate that fired), not once. `cnt_reject_decisions` above is a **count of rejected intents**, not gate-fires — measured as the number of `0x11` reject-diagnostic frames (FR-44, `cfg_reject_report=1`) emitted, since order_builder emits exactly one per blocked intent (carrying the winning `reject_reason`), independent of how many gates fired. `Σ(cnt_rej_*) ≥ cnt_reject_decisions` always, with equality only when no intent in the run had a multi-gate reject. Do not "fix" this by changing `cnt_rej_*` to count only the winning gate — that would directly break FR-43 and the coverage goal that every gate is independently demonstrated (§11.5).
...
All user-specified constraints are met. Worst setup path: `u_ob/tx_payload_reg[8]/C` → `u_hist/hist_mem_reg_r1_0_63_27_29/RAMB/I` (order_builder's TX payload register into the latency histogram's distributed-RAM write port) — see `docs/design_decisions.md` D26 through D40 for the five-bottleneck history that closed this. **Caveat:** 0.141 ns on an 8 ns period is 1.8% margin — real, but thin enough that any further change (S4's real classifier included) is likely to reopen it. This is exactly what happened: D54's reset-fanout fix (19 new physical registers) shifted placement congestion enough to reopen this same path family at a different bit index (`u_ob/tx_payload_reg[14]/C` → `u_hist/hist_mem_reg_r2_0_63_24_26/RAMA/I`, `WNS=-0.057ns`) — see `docs/design_decisions.md` D54. `constraints/tob_timing.xdc` sets no `set_input_delay`/`set_output_delay` on RGMII, so these 31,058 timed endpoints are internal paths only; the source-synchronous PHY interface is unconstrained and unmeasured here, and is the most likely place S11 stalls. **This table is D40's build and predates D52's real trained weights, D53's fix, and D54's fix+new-finding — the current build's WNS is negative (see `docs/design_decisions.md` D53/D54 and `results/timing.md`); re-run `make synth` before relying on these numbers.**
