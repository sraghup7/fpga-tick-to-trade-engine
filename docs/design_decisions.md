...
## D52 — Vaibhav's S4 ML training pipeline (`origin/develop`) merged, six correctness/integration bugs found and fixed, wired into the RTL side end to end

**Status: merged, all bugs fixed, real trained weights now in production `.mem` files, RTL bit-exactness harness built, CSR bring-up script built.** Closes the S4 gap CLAUDE.md's "What this repo is" section previously described as "has not started" — `model/train.py`'s pipeline (feature extraction, labeling, normalization, quantization, threshold selection, export) now runs end to end and its output is verified bit-exact against the RTL fallback classifier (`rtl/ml_classifier_wrap.v`) for every one of 4,860 real validation vectors.

**Background.** Vaibhav's ML branch (`origin/develop` commits `184c9e1`/`e207750`) was developed in isolation, forking from a point before this branch's S8-S11 RTL-side contract (`sim/feature_golden.py`, `sim/ml_golden.py`, the `$readmemh` hex `.mem` format) existed. A 2026-09-09 code review of that branch found 10 issues before merging; this entry records the fixes actually applied, in the order they landed:

1. **`model/ml_golden.py`'s `FeatureEngine` now delegates to `sim/feature_golden.py`'s `FeatureTracker`** (the RTL's own bit-exact reference) instead of an independent reimplementation. The independent copy had two real bugs: it never advanced the shared F5/F7 window on `trade` events, and it emptied (instead of correctly reseeding) the window on a `clear` event. Delegating to the RTL's own reference eliminates the possibility of the two sides drifting apart on this logic again.
2. **`compute_labels` no longer lets its `H`-event lookahead cross from one synthetic symbol's scenario into the next's** (the dataset concatenates ~140 symbols' scenarios back-to-back) — a `symbol_ids` check was added and the function vectorized in the same pass.
3. **Training now covers every valid event, not just the ~1/6 where the deterministic signal rule fires.** `rtl/feature_extractor.v` computes ML features on every book-modifying update, not only signal-firing ones, so restricting training/eval to signal rows meant the classifier (and its exported bit-exactness golden vectors) never exercised most of what the RTL actually scores at runtime. No-signal rows now get the well-defined label `y=0`.
4. **`hysteresis_policy` resets its Schmitt-trigger state at every symbol boundary**, matching `ml_policy.v`'s actual per-symbol `adverse_risk` state (D47) — the training-side diagnostic previously let one validation symbol's latched state leak into the next, unrelated symbol's first row.
5. **`pick_thresholds` is guarded against a genuine degenerate case**: if the validation set's overall positive rate is itself `>= TARGET_PRECISION`, the original cumulative-precision sweep could pick `t_high` at (or near) the minimum observed score, a risk gate that fires on almost everything. The fix checks that specific condition directly (`base_rate >= TARGET_PRECISION`) rather than an earlier, incorrect attempt that arbitrarily restricted the search to the top half of the validation set by score — that first attempt was caught by review because it silently clipped a separate, legitimate monotonic threshold-crossing scenario; the shipped fix has no such gap (proved algebraically: whenever the guarded branch does NOT fire, `precision_at_k` at `k=N` equals the base rate exactly, so the degenerate case is structurally unreachable).
6. **`model/train.py`'s `export()` now writes `weights.mem`/`bias.mem` as `$readmemh`-format two's-complement hex**, not decimal — this was the most severe finding of the original review: `rtl/ml_classifier_wrap.v`'s `$readmemh` call and `sim/ml_golden.py`'s own loader both require hex, and the decimal export either fatal-errored Verilog elaboration or silently corrupted values (confirmed by actually reproducing both failure modes against Icarus before this fix landed). `model_config.json` also now persists the proxy-label caveat, `cfg_min_spread`/`cfg_imb_shift`, and `weight_quant_headroom` as real fields, not just a console `print`.

**Regenerated real trained artifacts** (`model/model_config.json`, `weights.mem`, `bias.mem`, `normalization.mem`, `thresholds.mem`, `golden_vectors.csv`) by actually running the fixed `train.py`: 23,200 synthetic events, 16,200 in-horizon labeled rows, 3,681 with a deterministic trading signal (proving fix #3 above took effect — the labeled set is strictly larger than the signal-only subset), thresholds `T_high=-454`/`T_low=-458` (this run's validation positive rate never reached `TARGET_PRECISION`, so `pick_thresholds` took the `percentile(z_val, 95)` fallback branch directly rather than exercising the guarded precision-at-k search -- fix #5's degeneracy guard is verified by the dedicated unit test and the algebraic proof above, not by this particular training run), quantized weights `[4,-80,14,44,7,-114,26,-52]`, bias `-1302`.

**Added a permanent cross-check** (`model/tests/test_cross_check_sim_ml_golden.py`) feeding all 4,860 real golden vectors through both `model/ml_golden.py`'s `classify()` and `sim/ml_golden.py`'s `MLClassifier` (the two independently-maintained implementations computing the same `z = bias + sum(w_i*x_i)`) — 100% agreement, guarding against the two drifting apart the way they already once had (bugs #1 above existed only on the `model/` side).

**Built the RTL-side bit-exactness harness the master spec's test table already named but nothing implemented** (`T35_ml_bit_exact`, FR-34): `scripts/gen_ml_bit_exact_vectors.py` converts `model/golden_vectors.csv` into `$readmemh` stimulus, and `tb/tb_ml_bit_exact.v` replays all 4,860 real trained vectors through the actual synthesizable `rtl/ml_classifier_wrap.v`, confirming `z` bit-exactness. Scope decision: this covers `z` only, not `ml_policy.v`'s hysteresis state machine — that mechanism is generic per-slot logic already covered by `tb/tb_ml_policy.v`'s directed rising/falling/hold-zone/fail-safe cases, and re-deriving per-symbol reset points for ~4,860 concatenated validation rows to re-test the same mechanism would be duplicate coverage, not new signal.

**Discovered and fixed a real regression this same work introduced**: swapping `model/weights.mem`/`bias.mem` from the placeholder (`w_i=1`, `bias=0`) to real trained values broke four pre-existing testbenches (`tb_ml_classifier_wrap.v`, `tb_ml_chain.v`, `tb_tob_top.v`, `tb_top.v`) plus one Python hand-case (`sim/test_ml_golden_handcase.py`) and one soak-vector generator (`sim/gen_top_soak_vectors.py`) — all six hardcode expected outputs computed by hand assuming the OLD placeholder weights, but loaded weights via each module's default `WEIGHTS_FILE`/`BIAS_FILE` parameters (pointing at the now-changed live files). Fixed by adding a dedicated, fixed placeholder-weight fixture (`tb/stimulus/ml_placeholder_weights.mem`/`ml_placeholder_bias.mem`, byte-identical to the old placeholder) and overriding each affected module's weight-file parameters to point at it instead. `rtl/tob_top.v` gained two new pass-through parameters (`WEIGHTS_FILE`/`BIAS_FILE`, defaulting to the unchanged production values `"model/weights.mem"`/`"model/bias.mem"`) following the exact same established pattern this file already used for its `PHY_RESET_HOLD_CYCLES` simulation-only override — zero behavior change for any instantiation that doesn't override them. One of the six affected files (`sim/gen_top_soak_vectors.py`) was initially missed in the first fix attempt; the omission was caught not by re-reading the diff but by independently forcing a fresh (non-stale-cache) re-run of the full test suite, which is the only way the bug was actually observable — the first attempt's own "all tests passed" claim had silently relied on a stale, pre-real-weights cached stimulus file left on disk.

**Built the CSR bring-up script** (`scripts/load_ml_model.py`) that was entirely missing before this work: `feature_normalizer.v`'s and `ml_policy.v`'s normalization offsets/shifts and hysteresis thresholds are runtime CSR-writable registers (already fully wired end-to-end in `csr_block.v`/`tob_top.v` since the S6 milestone), but nothing loaded the *trained* values from `model_config.json` into them — this script sends the 18 required UDP CSR-write frames (register addresses independently verified byte-for-byte against `rtl/csr_block.v`'s actual write-decode logic, not just the master spec's register-map table). Weights/bias are deliberately NOT covered by this script — they remain elaboration-time-baked via `$readmemh` (FR-32), not CSR-writable; swapping them means re-synthesizing with new `.mem` files.

**Not done, explicitly out of scope for this pass:** the real hls4ml IP (`hls4ml/` is still empty; `ml_classifier_wrap.v`'s hand-written fallback, per master spec §15, remains what actually runs in gate `0x09`) and any change to `rtl/ml_classifier_wrap.v`'s own internals (only its two `.mem` inputs changed).

**Independently verified:** `RUN_SIM_FAST=1 bash scripts/run_sim.sh` passes (`ALL TESTS PASSED`) from a genuinely fresh checkout state (gitignored stimulus caches force-deleted and confirmed to regenerate, not reuse stale pre-fix output, before the final passing run was accepted); `model/tests/` (28 tests, including the previously-skipped `golden_vectors_consistency` suite and the new cross-check test) all pass against the real regenerated artifacts.

---

## D53 — ML classifier synthesis timing closure: balanced-tree + narrowed accumulator (necessary but not sufficient) plus 2-stage pipelining (sufficient); `SNAPSHOT_DEPTH` wiring gap fixed; retrain-timing policy adopted

**Status: the ML classifier's own path (`u_norm`/`u_ml` hierarchy) no longer appears among any violated path in `results/build/timing_summary.rpt`. Two other, unrelated timing failures remain open in the same build and are explicitly NOT addressed here — see "Out of scope" below.** This entry consolidates the design spec `docs/superpowers/specs/2026-09-09-ml-classifier-timing-closure-design.md` and its four implementation commits, all four of which referenced "D53" in their commit messages before this entry existed: `55fc0f9` (balanced-tree reassociation + `ACC_W=20` narrowing), `f9482da` (Python-side bound check in `model/train.py`), `813538c` (`SNAPSHOT_DEPTH` wiring fix), and `c15abd5` (2-stage pipelining). It replaces those commits' dangling references with one real entry, per the resolution both the spec and the task brief called for — do not split this work across multiple D-numbers.

**Root cause (confirmed against the actual routed netlist, not assumed).** D52 replaced `ml_classifier_wrap.v`'s placeholder weights (`w_i=1`, `bias=0`, which Vivado constant-folded away entirely — `x_i * 1` synthesizes to nothing but wire-forwarding) with real trained weights (`[4, -80, 14, 44, 7, -114, 26, -52]`, bias `-1302`). No RTL changed, but the previously-invisible multiply-accumulate became real logic, and re-synthesizing against it broke timing closure (`rx_clk` WNS `-3.842ns`, 342 failing endpoints). Two independent, additive causes, both verified directly against `results/build/timing_summary.rpt`'s netlist rather than assumed: (1) `z_c`'s 8-way sum was a literal left-to-right serial adder chain — one `CARRY4`-based adder per `+` in the Verilog source, matching the expression exactly, costing roughly 1.2ns per stage across 7 stages, because Vivado does not rebalance adder expressions on its own; (2) the `signed [31:0] z` accumulator was 15 bits wider than the true bound on `|z|` for any int8 weight set requires (`|bias| + Σ128·|w_i| = 44,950`, representable in 17 signed bits), and the don't-care sign-extension bits cost real carry-ripple delay because Vivado's range analysis doesn't see through the 8-deep serial chain to prove them constant. **A DSP-inference gap was investigated and explicitly ruled out**, not assumed: the weights are loaded via `$readmemh` into a `reg` array with no write port, so Vivado treats them as elaboration-time constants and constant-folds each multiply into a shift-add/`CARRY4` network rather than inferring a `DSP48E1` (confirmed in the netlist — `u_ml/p7__0_carry__0` is a constant-multiply carry structure for `x7 * (-52)`, not a generic multiplier); forcing `DSP48E1` mapping would not have helped even if it worked (a combinationally-used `DSP48E1` has a 4-5ns A→P delay on this speed grade, worse than the constant shift-add already present, and running it registered at 125MHz would cost 2 cycles inside the DSP itself). A `PCIN`/`PCOUT` DSP cascade was also considered and rejected as the wrong tool for 8 constant weights (would consume the entire 8-DSP budget for hand-instantiated primitives, against this project's inference-only convention, and cost roughly one pipeline register per cascaded DSP to run at speed) — the right answer for a much wider, runtime-configurable feature vector, not this one.

**The fix actually applied required both of the plan's two RTL changes — the zero-latency fix alone was not enough, contrary to what the original spec's "minimum stages that close timing" framing hoped for.** The zero-latency fix (balanced binary tree reassociation of the 8-product sum, narrowed to a `localparam integer ACC_W = 20` accumulator with headroom to `8·128·128 = 131,072 < 2^19`, sign-extended back to the full 32-bit `z` only at the very end — bit-exact by construction, since reassociating two's-complement addition is exact as long as no intermediate term overflows its declared width, and narrowing-then-sign-extending is exact iff the `ACC_W` bound holds) landed first and changed zero interface, zero latency, zero downstream contract. A targeted re-synthesis check afterward found it was **not sufficient on its own**: the ML path's worst slack got slightly *worse* (`-4.473ns`, vs. the original `-3.842ns`), because Vivado implements each `int8×int8` constant multiply (`p0..p7`) as its own `CARRY4` chain — a depth entirely separate from, and never addressed by, the accumulation-tree reassociation (the failing path ran *inside* one product term's own carry chain, e.g. `u_ml/p4_carry`/`u_ml/p4_carry__0`, before ever reaching the balanced tree's own `acc_carry` chains). Splitting `ml_classifier_wrap.v` into 2 registered pipeline stages — stage 1 registers the 8 raw products (`p0_r..p7_r`) straight from the normalizer's inputs, stage 2 computes the same balanced tree/narrowed accumulator from those registered products and registers the result into `z`/`ml_valid`/`ml_slot` — then closed it: final `rx_clk` WNS `-0.752ns` (better even than the original pre-fix `-3.842ns` baseline), TNS `-11.529ns` (down from `-190.913ns`), setup-failing endpoints 28 (down from 348), and no `u_norm/`/`u_ml/` path appears among the only 2 remaining `Slack (VIOLATED)` paths in the fresh `results/build/timing_summary.rpt`. This adds exactly 1 cycle: `ml_valid` now lands 2 cycles after `norm_valid` instead of 1.

**`SNAPSHOT_DEPTH`'s wiring gap, found and fixed independently of whether pipelining ended up being needed.** `rtl/tob_top.v`'s `ml_policy` instantiation passed no parameter override at all, silently relying on `ml_policy.v`'s own default `SNAPSHOT_DEPTH = 4` numerically happening to equal `ALIGN_DEPTH` — asymmetric with the adjacent `risk_engine` instantiation, which already does explicit `.ALIGN_DEPTH (ALIGN_DEPTH)` wiring with a comment that it must match. Fixed by adding the equivalent explicit `.SNAPSHOT_DEPTH (ALIGN_DEPTH)` wiring to `ml_policy`'s instantiation. This was a real latent bug independent of the timing-closure work — harmless only by coincidence, because both constants happened to be numerically equal until this same plan changed one of them.

**`ALIGN_DEPTH` (`rtl/tob_top.v`) and `ml_policy.v`'s `SNAPSHOT_DEPTH` are now 5 (were 4)**, a direct consequence of the pipelining fix above (the ML branch grew by 1 cycle; the signal-branch alignment delay must grow with it to keep `adverse_risk`'s registered value and the delayed signal-branch order intent referring to the same triggering event). Because of the `SNAPSHOT_DEPTH` wiring fix immediately above, `ml_policy` picks this up automatically from `ALIGN_DEPTH` with no separate edit needed. Cascading updates: `rtl/tob_top.v`'s `ALIGN_DEPTH` localparam and its explanatory comment; testbenches with hardcoded cycle-offset expectations (`tb/tb_ml_classifier_wrap.v`, `tb/tb_ml_chain.v`, `tb/tb_ml_policy.v`, `tb/tb_tob_top.v`, and `tb/tb_ml_bit_exact.v` — the last one not originally listed in the pipelining task's own file list but required for the same reason, since it also asserts a `norm_valid`→`ml_valid` cycle offset); `docs/contracts/ml_integration.md` §1.3's pipeline-timing table and `fpga_tick_to_trade_master_spec.md`. Full verification after the pipelining change: `iverilog -g2001 -Wall` clean with zero warnings; `tb_ml_classifier_wrap.v`, `tb_ml_bit_exact.v` (4,860/4,860 golden vectors bit-exact), `tb_ml_chain.v`, and `tb_ml_policy.v` (at both `SNAPSHOT_DEPTH=3` and `=5`) all pass; the full `bash scripts/run_sim.sh` (including the 1,000,000-message parser soak) reports `ALL TESTS PASSED`.

**Accepted policy: retrain-timing risk.** Because the classifier's weights are elaboration-time constants (`$readmemh`-loaded, FR-32, never CSR-writable) and different weight *values* produce different constant-multiply shift-add synthesis structures, **a future retrain can change synthesis timing with no RTL change at all** — a real, accepted tension with this project's fixed-cycle-count design goal, exactly as demonstrated by this same entry (D52's retrain broke timing that D26-D40 had previously closed, purely by changing `.mem` file contents). Decision (confirmed with the project owner, recorded here per the design spec): accept this, and require "re-synthesize and re-check timing" as an explicit manual step after any retrain — documented in `make ml`'s own description (`Makefile`, `CLAUDE.md`, `README.md`), not automated into the `make ml` target itself, since that target's job is training + golden-vector export and `make synth` already exists as a separate, correctly-scoped step. The `ACC_W` bound assertion (Phase 1 above, mirrored in both `rtl/ml_classifier_wrap.v`'s `initial`-block check and `model/train.py`'s `.mem`-writing path) is a narrower, complementary safeguard against actual numeric overflow, not a substitute for this timing policy — a retrain can stay within the `ACC_W` bound and still change synthesis timing.

**Reference:** the full root-cause analysis, alternatives considered, and phased fix plan are in `docs/superpowers/specs/2026-09-09-ml-classifier-timing-closure-design.md`.

**Explicitly out of scope (unchanged by this work, not silently omitted):** two other, unrelated timing failures remain in the current build, visible as the only 2 remaining `Slack (VIOLATED)` paths in `results/build/timing_summary.rpt`: (1) an async-reset-fanout recovery-check path, `u_rst_sync/ff2_reg/C` → a CSR counter's `CLR` input, in the `**async_default**` path group; (2) the `risk_engine` token-bucket path, `u_risk/refill_ctr_reg[1]/C` → `u_risk/token_bucket_reg[31]/D`. Neither has anything to do with the ML classifier; both need independent investigation. Do not read this entry, or any build produced by it, as a build-wide `make synth` PASS — only the ML classifier's own path is closed. Also out of scope: the real hls4ml IP (unaffected either way — this fix applies only to the hand-written fallback classifier that actually runs in gate `0x09`) and constraining future model quantization to keep weights "timing-cheap" (a rejected alternative policy; may be revisited if the accepted retrain-timing policy proves too disruptive in practice).

---

## D54 — Risk-engine token-bucket + reset-fanout timing closure: algebraic restructuring (necessary and sufficient) plus reset-tree replication (necessary; `phys_opt_design` alone proved non-durable); a third, pre-existing path reopened as a side effect and is left explicitly open

**Status: both of D53's named "explicitly out of scope" violations are durably closed and no longer appear as `Slack (VIOLATED)` in `results/build/timing_summary.rpt`. The build is still not gate-passing (`WNS = -0.057ns`) — a third, previously-latent path unrelated to either fix tipped negative as a side effect of this work's placement changes. See "Newly exposed, explicitly out of scope" below; do not read this entry as a build-wide `make synth` PASS.** Full derivation, case-analysis proofs, and the implementation plan's task-by-task record are in `docs/superpowers/plans/2026-09-10-risk-engine-timing-closure.md`; the RTL contract for the restructured arithmetic is `docs/contracts/risk_engine.md` §2.3.

**Violation 1 root cause and fix — `risk_engine.v` token bucket (`u_risk/refill_ctr_reg[1]/C` → `u_risk/token_bucket_reg[31]/D`, `-0.752ns` at D53).** The routed netlist showed two chained 32-bit ripple-carry structures evaluated combinationally in one cycle with no register between them: an incrementer+comparator computing `refill_tick`, immediately feeding a second carry chain computing `token_bucket`'s next value (an add followed by a conditional subtract on the same value). `cfg_token_max`/`cfg_token_refill_cycles` are runtime-writable 32-bit CSR registers, not elaboration-time constants, so Vivado could not constant-fold either compare — the fix had to restructure the arithmetic itself. Fixed by storing `refill_ctr+1` as the maintained invariant (`refill_ctr_p1`, renamed from `refill_ctr` — nothing outside `risk_engine.v` probed it by name) instead of `refill_ctr`, moving the "+1" off the forward comparison path and onto the register's own next-state update; `token_bucket_next` collapsed to a single 4-way mux (`inc`/`accepted_c`) replacing the previous chained refill-then-decrement. Proven bit-exact by direct case analysis across all four `(inc, accepted_c)` combinations against the original expressions — no pipelining, no added latency, same-cycle decisions throughout, preserving `sim/golden_model.py`'s exact-cycle refill/accept timing (D51's own concern). Verified with a continuous equivalence-check block added to `tb/tb_risk_engine.v` (shadow logic computing the original, pre-fix expressions every cycle and comparing against the DUT's actual `token_bucket`) plus two new boundary-value tests (`cfg_token_max=0`, `cfg_token_refill_cycles=1`): zero mismatches. Result: this path no longer appears among violated or worst paths in any post-fix build.

**Violation 2 root cause and fix — async-reset-fanout recovery path (`u_rst_sync/ff2_reg/C` → a CSR counter's `CLR`, `-0.012ns` at D53).** `rtl/common/sync_2ff.v`'s single 2FF-synchronizer output (`engine_rst_n`, instantiated as `u_rst_sync` in `rtl/tob_top.v`) drove roughly 19 module instances' async resets directly, producing one net with `fo≈10,876–10,881` after synthesis's active-low→active-high inverting LUT — a Recovery-type violation dominated by ROUTE delay, not logic depth. The first attempted fix — adding a missing `phys_opt_design` call to `scripts/build.tcl` (which includes automatic high-fanout net replication) — passed on the first re-synthesis (`0.528ns` margin) but direct `vivado.log` evidence ("No setup violation found. The netlist was not modified.") showed `phys_opt_design` made zero netlist changes: `place_design` had already reached `WNS≥0` before `phys_opt_design` ran, and `phys_opt_design` structurally skips all setup optimization once WNS is already non-negative — this was placement luck, not a durable fix, and the target net's fanout was still `fo=10876`, essentially untouched. The durable fix replicated `engine_rst_n` into 19 local per-module reset buffer registers in `rtl/tob_top.v`, each declared `(* keep = "true" *)` (preventing synthesis from merging them back into one signal) and each independently async-cleared by `engine_rst_n` (`always @(posedge gmii_rx_clk or negedge engine_rst_n)`). **Correction (found during the whole-branch review, not by either Task 4's implementer or its task review): this is not zero-latency.** Assertion is cycle-identical (async, immediate, same as before). Release is not: because each buffer's release value (`1'b1`) is captured in the block's `else` branch, a buffer's own output only rises on the *next* `gmii_rx_clk` edge after `engine_rst_n` itself has already risen — a genuine one-cycle lag, confirmed directly in simulation (a counter gated by a buffer starts incrementing exactly one clock period later than an otherwise-identical counter gated by `engine_rst_n` directly). What makes this safe is not the absence of delay but its *uniformity*: all 19 buffers share this one `always` block, so every consumer's reset releases on the same cycle as every other consumer's — every intra-design relative-timing invariant this project depends on (most importantly D51's `refill_ctr`/`cur_cycle` lockstep, both now gated by buffers from this same block) shifts together and stays intact. `bash scripts/run_sim.sh`'s continued `ALL TESTS PASSED` after this change is consistent with — not proof of — zero-latency, since neither the golden model's own cycle counter nor the soak's directed timing is anchored to reset-release wall-clock time in a way that would expose a uniform shift; it is evidence that the *relative* invariants held, which is what actually matters. `rtl/tob_top.v`'s own comment on this block and `sim/golden_model.py`'s D51 comment have been corrected to state this accurately. This is a pure fanout/placement change with zero change to any *relative* intra-design timing. Verified genuinely fixed (not lucky) on re-synthesis: the reset-recovery path's source is now one of the new buffers (`rst_n_feat_reg`), fanout down to `fo=3509` (a real ~3.1x reduction, not the ~19x initially estimated, since `feature_extractor.v` already owned a disproportionate share of the design's async-reset registers), and the path MEETS timing at `0.395ns`/`0.339ns` margins — confirmed via a fresh `results/build/timing_summary.rpt` showing zero VIOLATED paths in the `**async_default**` path group.

**Newly exposed, explicitly out of scope: a third, pre-existing fragile path reopened.** Adding 19 new physical registers (Violation 2's fix) shifted overall placement/routing congestion enough to tip a third, previously-passing path negative: `u_ob/tx_payload_reg[14]/C` → `u_hist/hist_mem_reg_r2_0_63_24_26/RAMA/I` (`order_builder` → `latency_histogram`'s RAM write port), `Slack (VIOLATED) -0.057ns`, the only failing endpoint in the current `results/build/timing_summary.rpt` (`WNS = -0.057ns`, `TNS = -0.057ns`, 1 of 31,649 endpoints; `WHS = +0.044ns`, hold met). This is not a defect in either fix above — it is the exact path family `fpga_tick_to_trade_master_spec.md` §12.3 already flagged at D40 (`u_ob/tx_payload_reg[8]/C` → `u_hist/hist_mem_reg_r1_0_63_27_29/RAMB/I`, `+0.141ns` margin) with an explicit caveat that "any further change is likely to reopen it" — which is exactly what happened here, one bit index over. It is a different problem domain (RAM-write address/data timing inside `order_builder`/`latency_histogram`, not token-bucket arithmetic or reset fanout) and was deliberately **not** absorbed into this plan's scope, matching this project's established pattern of flagging newly-discovered out-of-scope findings rather than silently expanding an already-running plan (D45; this same pattern is how D53's own "explicitly out of scope" list produced this very plan). `results/build/tob_top.bit` remains absent/stale — `scripts/build.tcl`'s `WNS < 0` gate correctly still refuses `write_bitstream`. This path needs its own root-cause investigation and is left as a new, separate open item.

**This closes out D53's "explicitly out of scope" item for the two named violations; it does not close out D53's broader statement that the build is gate-passing** — that remains false, now for a different, single reason than at D53.

**Verification (full accounting).** `iverilog -g2001 -Wall`: zero warnings, zero inferred latches. `bash scripts/run_sim.sh` (fast and full, including the 1,000,000-message parser soak): `ALL TESTS PASSED`, both before and after each task's changes, with zero regressions. `make synth` run to genuine completion (verified via `vivado.log`'s real completion sequence, not assumed) after each of Tasks 1, 2/3, and 4.
