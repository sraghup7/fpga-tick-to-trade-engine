...
**Regenerated real trained artifacts** (`model/model_config.json`, `weights.mem`, `bias.mem`, `normalization.mem`, `thresholds.mem`, `golden_vectors.csv`) by actually running the fixed `train.py`: 23,200 synthetic events, 16,200 in-horizon labeled rows, 3,681 with a deterministic trading signal (proving fix #3 above took effect — the labeled set is strictly larger than the signal-only subset), thresholds `T_high=-454`/`T_low=-458` (nowhere near the validation set's true minimum, some sample rows go below `-1000`, proving fix #5 is not degenerate on real data), quantized weights `[4,-80,14,44,7,-114,26,-52]`, bias `-1302`.
...
**Independently verified:** `RUN_SIM_FAST=1 bash scripts/run_sim.sh` passes (`ALL TESTS PASSED`) from a genuinely fresh checkout state (gitignored stimulus caches force-deleted and confirmed to regenerate, not reuse stale pre-fix output, before the final passing run was accepted); `model/tests/` (27 tests, including the previously-skipped `golden_vectors_consistency` suite and the new cross-check test) all pass against the real regenerated artifacts.
