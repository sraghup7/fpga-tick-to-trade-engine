...
## What this repo is
- **Implementation in progress, milestones S0–S3/S5–S10 done, S11 (hardware bring-up) blocked on timing closure** (as of 2026-09-05). `rtl/`, `sim/`, `tb/`, `docs/contracts/` are populated; every landed module has a passing Icarus testbench, plus passing golden-model hand-cases (`golden_model.py`/`feature_golden.py`/`ml_golden.py`) and a passing 1M-message parser soak. `ml_classifier_wrap.v`/`ml_policy.v` (gate `0x09`), the S6 alignment register, S9's `latency_histogram.v`/`csr_block.v`, and S10's board-level `tob_top.v` all exist and pass simulation. `model/` holds a documented, untrained placeholder classifier (`w_i=1`) — **S4 (real ML training/hls4ml) hasn't started**, `hls4ml/` is still empty. **S11 is blocked**: real Vivado synthesis found `feature_extractor.v` fails timing and `risk_engine.v` has a real cross-module alignment bug (fix contracts in flight, `docs/design_decisions.md` D26-D30 have the full trace); no bitstream exists yet. `tb/tb_top.v` (full-system test) doesn't exist. Check with `ls` before assuming a specific file exists — `docs/design_decisions.md` (31 entries) is the authoritative record of what's actually landed, this line drifts fast. `make sim` (→ `scripts/run_sim.sh`) runs the full regression (golden models, RTL lint, every testbench, the 1M-message soak) and is what CI runs on every push; `make synth`/`make bit` (→ `scripts/build.tcl`, real Vivado non-project-mode) currently fail their own latch/timing gates on purpose; `make ml` is still a stub (S4, ML collaborator's side). `sim/order_rx.py` decodes order-record frames (offline hex capture or live UDP); `--selftest` is its own hand-case check. The eventual full code layout is defined in `fpga_tick_to_trade_master_spec.md` §13.
