...
## What this repo is
- **Implementation in progress, milestones S0–S3/S5/S7/S8 done** (as of 2026-09-03). `rtl/`, `sim/`, `tb/`, `docs/contracts/` are populated; every landed module has a passing Icarus testbench, plus passing golden-model hand-cases and a passing 1M-message parser soak. `model/`/`hls4ml/` are still empty — S4/S6 (ML) and S9 (`latency_histogram.v`/`csr_block.v`)/S10 (`tb/tb_top.v`) haven't started. Check with `ls` before assuming a specific file exists. `make sim` (→ `scripts/run_sim.sh`) runs the full regression (golden models, RTL lint, every testbench, the 1M-message soak) and is what CI runs on every push; `make ml` is still a stub (S4, ML collaborator's side). `sim/order_rx.py` decodes order-record frames (offline hex capture or live UDP); `--selftest` is its own hand-case check. The eventual full code layout is defined in `fpga_tick_to_trade_master_spec.md` §13.
