...
**Implementation in progress, milestones S0–S3/S5–S10 done; S11 (hardware bring-up) has a real, gate-passing bitstream for the first time (`results/build/tob_top.bit`) but has not yet been tried on physical hardware** (as of 2026-09-05). `rtl/`, `sim/`, `tb/`, and `docs/contracts/` are populated; every landed module has a passing testbench (Icarus) plus golden models (`sim/golden_model.py`, `sim/feature_golden.py`, `sim/ml_golden.py`) with passing hand-case regressions; the 1,000,000-message parser soak (`tb/tb_parser_soak.v`) passes. `ml_classifier_wrap.v`, `ml_policy.v` (gate `0x09`'s hysteresis policy), the S6 alignment register (`ALIGN_DEPTH` in `tob_top.v`), S9's `latency_histogram.v`/`csr_block.v`, and S10's board-level `tob_top.v` all exist and pass simulation. **S4's training/quantization pipeline has landed and been fixed** (`model/train.py`, `docs/design_decisions.md` D52) -- `model/weights.mem`/`bias.mem` are now real trained, quantized weights (not the `w_i=1` placeholder), loaded via the same `$readmemh` contract `rtl/ml_classifier_wrap.v` always had. `hls4ml/` itself is still empty -- the hand-written `ml_classifier_wrap.v` fallback (master spec §15) remains what actually runs in gate `0x09`; swapping in a real hls4ml IP is still open and does not require any RTL wiring change (only the `.mem` files / IP internals change, per the original S6 contract). **S11's build-flow blockers are all resolved**: `scripts/build.tcl` (a real `make synth`/`make bit` target, not a stub) now runs clean end to end — real Vivado synthesis/place/route closes timing (WNS/WHS both positive), 0 inferred latches, and `write_bitstream` completed successfully (D26 through D40, `docs/design_decisions.md`). `tb/tb_top.v` (full-system integration test) still does not exist — check current state with `ls` before assuming a specific file exists, this table drifts fast, and `docs/design_decisions.md` (40 entries as of this writing) is the authoritative log of what's actually been found and fixed, not this summary. `make sim` (→ `scripts/run_sim.sh`) runs every Python golden-model/hand-case check, an RTL lint, every per-module testbench, and the 1M-message soak, exiting non-zero on any failure — CI's `golden-model-sim` job runs the same script on every push. `make ml` runs the real training pipeline (`cd model && python train.py`, then regenerates the RTL bit-exactness vectors). `sim/order_rx.py` decodes the order-record wire format (S4.5) from either an offline hex capture or a live UDP socket; run `python sim/order_rx.py --selftest` for its own hand-case check.
