...
**Implementation in progress, milestones S0–S3/S5/S7/S8 done** (as of 2026-09-03). `rtl/`, `sim/`, `tb/`, and `docs/contracts/` are populated and every landed module has a passing testbench (Icarus) plus two golden models (`sim/golden_model.py`, `sim/feature_golden.py`) with passing hand-case regressions; the 1,000,000-message parser soak (`tb/tb_parser_soak.v`) passes. `model/` and `hls4ml/` are still empty — **S4 (ML training/hls4ml export) and S6 (ML integration) have not started**, so `ml_classifier_wrap.v`, `ml_policy.v`, and the alignment register don't exist yet, and neither does S9's `latency_histogram.v`/`csr_block.v` or S10's `tb/tb_top.v`. Check current state with `ls` before assuming a specific file exists — the table below is milestone-accurate as of this writing but drifts fast. Two caveats even for what exists: `make sim`/`make ml` are still stub echoes (no `run_sim.sh`/`build_hls4ml.py` yet — run testbenches directly with `iverilog`/`vvp` per module, or see `tb/README.md`), and CI's `golden-model-sim` job is still gated `if: false` in `.github/workflows/ci.yml` pending that script, so a green CI run today only proves the RTL lints, not that tests pass.
