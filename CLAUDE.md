...
**Implementation in progress, milestones S0–S3/S5–S10 done; S11 (hardware bring-up) had a real, gate-passing bitstream for the first time (`results/build/tob_top.bit`) as of 2026-09-05, but that specific artifact is currently stale/absent** — D52's real trained ML weights (2026-09-09) broke build-wide timing closure, and as of D53 (2026-09-10) `results/build/` has a fresh timing report but no `.bit` file: `scripts/build.tcl` correctly refuses to call `write_bitstream` while `WNS < 0` (currently `-0.752ns`, from two ML-unrelated violations — see D53), so a fresh, gate-passing bitstream does not exist again until those are independently fixed. `rtl/`, `sim/`, `tb/`, and `docs/contracts/` are populated; every landed module has a passing testbench (Icarus) plus golden models (`sim/golden_model.py`, `sim/feature_golden.py`, `sim/ml_golden.py`) with passing hand-case regressions; the 1,000,000-message parser soak (`tb/tb_parser_soak.v`) passes. `ml_classifier_wrap.v`, `ml_policy.v` (gate `0x09`'s hysteresis policy), the S6 alignment register (`ALIGN_DEPTH` in `tob_top.v`), S9's `latency_histogram.v`/`csr_block.v`, and S10's board-level `tob_top.v` all exist and pass simulation. **S4's training/quantization pipeline has landed and been fixed** (`model/train.py`, `docs/design_decisions.md` D52) -- `model/weights.mem`/`bias.mem` are now real trained, quantized weights (not the `w_i=1` placeholder), loaded via the same `$readmemh` contract `rtl/ml_classifier_wrap.v` always had. `hls4ml/` itself is still empty -- the hand-written `ml_classifier_wrap.v` fallback (master spec §15) remains what actually runs in gate `0x09`; swapping in a real hls4ml IP is still open and does not require any RTL wiring change (only the `.mem` files / IP internals change, per the original S6 contract). **S11's build-flow tooling is real, not a stub**: `scripts/build.tcl` (a real `make synth`/`make bit` target) runs real Vivado synthesis/place/route end to end with 0 inferred latches, and originally closed build-wide timing and completed `write_bitstream` (D26 through D40, `docs/design_decisions.md`), as of 2026-09-05. That closed-timing state does not currently hold, for reasons unrelated to `build.tcl` itself: D52's real trained ML weights (2026-09-09) turned a previously constant-folded-away multiply-accumulate into real logic and broke build-wide timing; D53 (2026-09-10) closed the ML classifier's own path specifically (balanced-tree reassociation + narrowed accumulator + 2-stage pipelining), but two other, ML-unrelated timing violations (an async-reset-fanout recovery path, and the `risk_engine` token-bucket path) remain open, so `build.tcl`'s own `WNS < 0` gate correctly refuses to write a bitstream right now — see D53 for the full accounting; do not assume `results/build/tob_top.bit` exists without checking. `tb/tb_top.v` (full-system integration test) still does not exist — check current state with `ls` before assuming a specific file exists, this table drifts fast, and `docs/design_decisions.md` (53 entries as of this writing) is the authoritative log of what's actually been found and fixed, not this summary. `make sim` (→ `scripts/run_sim.sh`) runs every Python golden-model/hand-case check, an RTL lint, every per-module testbench, and the 1M-message soak, exiting non-zero on any failure — CI's `golden-model-sim` job runs the same script on every push. `make ml` runs the real training pipeline (`cd model && python train.py`, then regenerates the RTL bit-exactness vectors) — because classifier weights are elaboration-time constants, a retrain can change synthesis timing with no RTL change at all (D53); `make synth` must be re-run and its timing report re-checked after any retrain, before trusting any existing bitstream. `sim/order_rx.py` decodes the order-record wire format (S4.5) from either an offline hex capture or a live UDP socket; run `python sim/order_rx.py --selftest` for its own hand-case check.

`fpga-tick-to-trade-engine`: a pipelined FPGA datapath (Artix-7 XC7A35T-2FGG484I, ALINX AX7035B board, JLSemi JL2121(D) PHY — see the correction note in `docs/design_decisions.md` D4 before trusting any older doc that still says "KSZ9031RNX") that receives a synthetic Gigabit-Ethernet market-data feed, parses and filters it, maintains top-of-book state, extracts fixed-point features, runs a quantized (hls4ml-generated) linear classifier that estimates adverse-selection risk, and gates the resulting order intent through nine non-bypassable pre-trade risk checks (including the ML verdict) before emitting a simulated order — all at a fixed, measured tick-to-trade cycle count, verified bit-exact against a Python golden model. Simulated trading only; never connects to a real exchange or handles money.
...
- Target device: Artix-7 `XC7A35T-2FGG484I` (ALINX AX7035B board, JLSemi JL2121(D) PHY, RGMII), single 125 MHz clock domain (8 ns period), Vivado 2023.x+ / Vitis HLS 2023.x+ (2024.2 verified locally, see `PREREQUISITES.md`). The board's own schematic (`docs/refs/AX7035/SCH/SCH.pdf`) and this repo's earlier docs say "KSZ9031RNX" — that's wrong for the physically populated chip; see `docs/design_decisions.md` D4 and `docs/refs/AX7035B_pinout_notes.md`.
