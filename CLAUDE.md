...
**Implementation in progress, milestones S0–S3/S5/S7/S8 done** (as of 2026-09-03). `rtl/`, `sim/`, `tb/`, and `docs/contracts/` are populated and every landed module has a passing testbench (Icarus) plus two golden models (`sim/golden_model.py`, `sim/feature_golden.py`) with passing hand-case regressions; the 1,000,000-message parser soak (`tb/tb_parser_soak.v`) passes. `model/` and `hls4ml/` are still empty — **S4 (ML training/hls4ml export) and S6 (ML integration) have not started**, so `ml_classifier_wrap.v`, `ml_policy.v`, and the alignment register don't exist yet, and neither does S9's `latency_histogram.v`/`csr_block.v` or S10's `tb/tb_top.v`. Check current state with `ls` before assuming a specific file exists — the table below is milestone-accurate as of this writing but drifts fast. `make sim` (→ `scripts/run_sim.sh`) runs every Python golden-model/hand-case check, an RTL lint, every per-module testbench, and the 1M-message soak, exiting non-zero on any failure — CI's `golden-model-sim` job runs the same script on every push. `make ml` is still a stub (owned by the ML collaborator, lands at S4). `sim/order_rx.py` decodes the order-record wire format (S4.5) from either an offline hex capture or a live UDP socket; run `python sim/order_rx.py --selftest` for its own hand-case check.
