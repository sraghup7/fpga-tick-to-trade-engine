...
**Implementation in progress, milestones S0–S3/S5–S10 done; S11 (hardware bring-up) had a real, gate-passing bitstream for the first time (`results/build/tob_top.bit`) as of 2026-09-05, but that specific artifact is currently stale/absent** — D52's real trained ML weights (2026-09-09) broke build-wide timing closure; D53 (2026-09-10) closed the ML classifier's own path; D54 (2026-09-10) closed the two violations D53 left open (the `risk_engine` token-bucket path and the async-reset-fanout recovery path), but D54's own fix exposed a third, previously-latent path (`order_builder`→`latency_histogram`'s RAM write, already flagged as thin at D40) that is now the build's only failure: `scripts/build.tcl` correctly refuses to call `write_bitstream` while `WNS < 0` (currently `-0.057ns` — see D54 for the full accounting), so a fresh, gate-passing bitstream does not exist yet. `rtl/`, `sim/`, `tb/`, and `docs/contracts/` are populated; every landed module has a passing testbench (Icarus) plus golden models (`sim/golden_model.py`, `sim/feature_golden.py`, `sim/ml_golden.py`) with passing hand-case regressions; the 1,000,000-message parser soak (`tb/tb_parser_soak.v`) passes. `ml_classifier_wrap.v`, `ml_policy.v` (gate `0x09`'s hysteresis policy), the S6 alignment register (`ALIGN_DEPTH` in `tob_top.v`), S9's `latency_histogram.v`/`csr_block.v`, and S10's board-level `tob_top.v` all exist and pass simulation. **S4's training/quantization pipeline has landed and been fixed** (`model/train.py`, `docs/design_decisions.md` D52) -- `model/weights.mem`/`bias.mem` are now real trained, quantized weights (not the `w_i=1` placeholder), loaded via the same `$readmemh` contract `rtl/ml_classifier_wrap.v` always had. `hls4ml/` itself is still empty -- the hand-written `ml_classifier_wrap.v` fallback (master spec §15) remains what actually runs in gate `0x09`; swapping in a real hls4ml IP is still open and does not require any RTL wiring change (only the `.mem` files / IP internals change, per the original S6 contract). **S11's build-flow tooling is real, not a stub**: `scripts/build.tcl` (a real `make synth`/`make bit` target) runs real Vivado synthesis/place/route end to end with 0 inferred latches, and originally closed build-wide timing and completed `write_bitstream` (D26 through D40, `docs/design_decisions.md`), as of 2026-09-05. That closed-timing state does not currently hold: see `docs/design_decisions.md` D52 through D54 for the full chain of what broke it, what's since been fixed, and what remains open (currently one path, `u_ob`→`u_hist`, at `WNS=-0.057ns`) — do not assume `results/build/tob_top.bit` exists without checking. `tb/tb_top.v` (full-system integration test) exists and passes (exercised by `scripts/run_sim.sh`) — this table has drifted on file-existence claims before, so check with `ls` rather than trusting a hardcoded claim here if it looks stale. `docs/design_decisions.md` (54 entries as of this writing) is the authoritative log of what's actually been found and fixed, not this summary. `make sim` (→ `scripts/run_sim.sh`) runs every Python golden-model/hand-case check, an RTL lint, every per-module testbench, and the 1M-message soak, exiting non-zero on any failure — CI's `golden-model-sim` job runs the same script on every push. `make ml` runs the real training pipeline (`cd model && python train.py`, then regenerates the RTL bit-exactness vectors) — because classifier weights are elaboration-time constants, a retrain can change synthesis timing with no RTL change at all (D53); `make synth` must be re-run and its timing report re-checked after any retrain, before trusting any existing bitstream. `sim/order_rx.py` decodes the order-record wire format (S4.5) from either an offline hex capture or a live UDP socket; run `python sim/order_rx.py --selftest` for its own hand-case check.
